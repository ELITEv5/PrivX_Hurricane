...
async function doShield() {
  ...
  noteCopied = false;
  const _copyBtn = document.getElementById('note-copy-btn');
  if (_copyBtn) { _copyBtn.textContent = 'Copy Note'; _copyBtn.style.background = ''; _copyBtn.style.color = ''; _copyBtn.style.borderColor = ''; }
...
function copyNote() {
  ...
  if (!noteCopied) {
    ...
    noteCopied = true;
    btn.textContent = '⚠ Saved it? Tap again to shield next';
    ...
    noteCopied = false;
    document.getElementById('note-reveal').style.display = 'none';
    document.getElementById('note-reveal-text').textContent = '';
...
// ABI-encode getAmountsIn(uint256 amountOut, address[] path) — reverse quote: how much PLS for exact stable out
function encodeGetAmountsIn(amountOut, path) {
  const h = v => BigInt(v).toString(16).padStart(64,'0');
  const a = addr => addr.slice(2).toLowerCase().padStart(64,'0');
  return '0x1f00ca74' + h(amountOut) + h(64) + h(path.length) + path.map(a).join('');
}

// Decode getAmountsIn result — returns first element (PLS in needed)
function decodeAmountsIn(hex) {
  const data = hex.startsWith('0x') ? hex.slice(2) : hex;
  return BigInt('0x' + data.slice(128, 192));
}

// ABI-encode swapExactTokensForTokens(uint amountIn, uint amountOutMin, address[] path, address to, uint deadline)
function encodeSwapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline) {
  const h = v => BigInt(v).toString(16).padStart(64,'0');
  const a = addr => addr.slice(2).toLowerCase().padStart(64,'0');
  // path is 3rd param (index 2), offset = 5 slots * 32 = 160
  return '0x38ed1739' + h(amountIn) + h(amountOutMin) + h(160) + a(to) + h(deadline) + h(path.length) + path.map(a).join('');
}

// Reverse quote: find the cheapest PLS path to receive exactly stableWei of tokenKey
async function fetchPLSForStable(stableWei, tokenKey) {
  const paths = SWAP_PATHS[tokenKey];
  let bestPls = null, bestPath = null;
  for (const path of paths) {
    try {
      const data   = encodeGetAmountsIn(stableWei, path);
      const result = await rpc('eth_call', [{ to: PULSEX_ROUTER, data }, 'latest']);
      const plsIn  = decodeAmountsIn(result);
      if (bestPls === null || plsIn < bestPls) { bestPls = plsIn; bestPath = path; }
    } catch { /* pair may not exist or insufficient liquidity */ }
  }
  if (!bestPath) throw new Error('No liquidity found for this pair on PulseX.');
  return { plsIn: bestPls, path: bestPath };
}

// Reverse quote: how much fromKey stable to get exactly stableWei of toKey stable
async function fetchTokenForStable(stableWei, fromKey, toKey) {
  const fromAddr = TOKENS[fromKey].addr;
  const toAddr   = TOKENS[toKey].addr;
  const daiAddr  = TOKENS.dai.addr;
  const candidates = [
    [fromAddr, WPLS, toAddr],
    [fromAddr, toAddr],
    [fromAddr, daiAddr, toAddr], // 3-hop via DAI for USDC↔pSunDAI
  ];
  let bestIn = null, bestPath = null;
  for (const path of candidates) {
    if (path[0] === path[path.length - 1]) continue; // skip trivial
    try {
      const data   = encodeGetAmountsIn(stableWei, path);
      const result = await rpc('eth_call', [{ to: PULSEX_ROUTER, data }, 'latest']);
      const inAmt  = decodeAmountsIn(result);
      if (bestIn === null || inAmt < bestIn) { bestIn = inAmt; bestPath = path; }
    } catch {}
  }
  if (!bestPath) throw new Error('No liquidity found for this pair on PulseX.');
  return { tokenIn: bestIn, path: bestPath };
}

// Ensure the PulseX router has sufficient allowance for a token; if not, send approve tx
async function ensureTokenApproval(tokenAddr, amount, statusEl) {
  const ownerHex  = primaryWallet.address.slice(2).toLowerCase().padStart(64,'0');
  const routerHex = PULSEX_ROUTER.slice(2).toLowerCase().padStart(64,'0');
  const allowData = '0xdd62ed3e' + ownerHex + routerHex;
  const result    = await rpc('eth_call', [{ to: tokenAddr, data: allowData }, 'latest']);
...
function renderSwapUI() {
  // From selector: PLS + all tokens
  const fg = document.getElementById('swap-from-grid');
  if (fg) {
    const fromOpts = [{ key:'pls', label:'PLS' }, ...Object.entries(TOKENS).map(([k,t]) => ({ key:k, label:t.label }))];
    fg.innerHTML = fromOpts.map(({ key, label }) =>
      `<div class="chip${key===swapFrom?' active':''}" style="flex:1;min-width:60px" onclick="selectSwapFrom('${key}')">
         <div class="cval" style="font-size:14px">${label}</div></div>`
    ).join('');
  }
  // To selector: exclude source token
  const tg = document.getElementById('swap-token-grid');
  if (!tg) return;
  tg.innerHTML = Object.entries(TOKENS)
    .filter(([k]) => k !== swapFrom)
    .map(([k,t]) =>
      t.comingSoon
        ? `<div class="chip chip-soon"><div class="cval">${t.label}</div><div style="font-size:9px;opacity:.55;margin-top:2px">SOON</div></div>`
        : `<div class="chip${k===swapToken?' active':''}" onclick="selectSwapToken('${k}')"><div class="cval">${t.label}</div></div>`
    ).join('');
}
...
function selectSwapFrom(k) {
  swapFrom = k;
  // if destination == new source, pick a different destination
  if (swapToken === swapFrom) {
    swapToken = Object.keys(TOKENS).find(key => key !== swapFrom && !TOKENS[key].comingSoon) || 'dai';
  }
  swapQuote = null;
  renderSwapUI();
  document.getElementById('swap-quote-box').style.display = 'none';
...
function setSwapDenom(d) {
  document.getElementById('swap-dollar-input').value = d;
  // highlight active denom btn
  document.querySelectorAll('.swap-denom-btn').forEach(b => {
...
function onSwapInput() {
  // clear active highlight when user types custom amount
  const val = document.getElementById('swap-dollar-input').value.trim();
  const matching = [1,5,10,20,50,100].includes(Number(val));
  document.querySelectorAll('.swap-denom-btn').forEach(b => {
    b.classList.toggle('active', matching && b.textContent === '$' + val);
  });
  swapQuote = null;
  document.getElementById('swap-quote-box').style.display = 'none';
  document.getElementById('swap-btn').disabled = true;
...
async function triggerSwapQuote() {
  const rawDollar = document.getElementById('swap-dollar-input').value.trim();
  if (!rawDollar || Number(rawDollar) <= 0) return;
  const toTok = TOKENS[swapToken];
  ...
  setStatus(st, '', '');
  try {
    const stableBase    = BigInt(Math.round(Number(rawDollar) * 10 ** toTok.decimals));
    const stableWithFee = stableBase * 10000n / 9950n;
    const feeWei        = stableWithFee - stableBase;
    const amountOutMin  = stableWithFee * 995n / 1000n;

    const fmtTo  = wei => '$' + (Number(wei) / 10 ** toTok.decimals).toLocaleString(undefined, { maximumFractionDigits: 4 }) + ' ' + toTok.label;
    const routeLabel = path => path.map(a => {
    ...
    let inputLabel, inputDisplay;

    if (swapFrom === 'pls') {
      const { plsIn, path } = await fetchPLSForStable(stableWithFee, swapToken);
      const plsWithBuffer   = plsIn * 101n / 100n;
      swapQuote = { mode:'pls', plsWei: plsWithBuffer, stableWithFee, feeWei, stableBase, amountOutMin, path };
      inputLabel   = 'PLS to send';
      inputDisplay = (Number(plsWithBuffer) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 0 }) + ' PLS';
      document.getElementById('swap-quote-route').textContent = routeLabel(path);
    } else {
      const fromTok = TOKENS[swapFrom];
      const { tokenIn, path } = await fetchTokenForStable(stableWithFee, swapFrom, swapToken);
      const tokenInWithBuffer = tokenIn * 101n / 100n;
      swapQuote = { mode:'stable', fromKey: swapFrom, tokenIn: tokenInWithBuffer, stableWithFee, feeWei, stableBase, amountOutMin, path };
      inputLabel   = fromTok.label + ' to spend';
      inputDisplay = '$' + (Number(tokenInWithBuffer) / 10 ** fromTok.decimals).toLocaleString(undefined, { maximumFractionDigits: 4 }) + ' ' + fromTok.label;
      document.getElementById('swap-quote-route').textContent = routeLabel(path);
    }

    document.getElementById('swap-quote-input-label').textContent = inputLabel;
    document.getElementById('swap-quote-pls').textContent         = inputDisplay;
    document.getElementById('swap-quote-stable').textContent      = fmtTo(stableWithFee);
    document.getElementById('swap-quote-fee').textContent         = fmtTo(feeWei);
    document.getElementById('swap-quote-net').textContent         = fmtTo(stableBase);
...
function reviewSwap() {
  ...
  const fmtTo  = wei => '$' + (Number(wei) / 10 ** toTok.decimals).toLocaleString(undefined, { maximumFractionDigits: 4 }) + ' ' + toTok.label;
  let sendStr;
  if (swapQuote.mode === 'pls') {
    sendStr = (Number(swapQuote.plsWei) / 1e18).toLocaleString(undefined, { maximumFractionDigits: 0 }) + ' PLS';
  } else {
    const fromTok = TOKENS[swapQuote.fromKey];
    sendStr = '$' + (Number(swapQuote.tokenIn) / 10 ** fromTok.decimals).toLocaleString(undefined, { maximumFractionDigits: 4 }) + ' ' + fromTok.label;
  }
  document.getElementById('sc-pls-out').textContent      = sendStr;
  document.getElementById('sc-stable-in').textContent    = fmtTo(swapQuote.stableWithFee);
  document.getElementById('sc-net-shielded').textContent = fmtTo(swapQuote.stableBase);
  ...
  document.getElementById('swap-quote-box').style.display  = 'none';
  document.getElementById('swap-dollar-input').disabled    = true;
...
function cancelSwapConfirm() {
  ...
  document.getElementById('swap-dollar-input').disabled    = false;
...
async function executeSwap() {
  ...
  try {
    ...
    if (swapQuote.mode === 'pls') {
      await preflightPls(swapQuote.plsWei + 2000000000000000000n);
      setStatus(st, 'Sending swap transaction…', 'info');
      const data = encodeSwapExactETHForTokens(swapQuote.amountOutMin, swapQuote.path, primaryWallet.address, deadline);
      tx = await primaryWallet.sendTransaction({ to: PULSEX_ROUTER, value: swapQuote.plsWei, data, gasLimit: 400000n, type: 0, gasPrice: swapGasPrice });
    } else {
      const fromTok = TOKENS[swapQuote.fromKey];
      await ensureTokenApproval(fromTok.addr, swapQuote.tokenIn, st);
      setStatus(st, 'Sending swap transaction…', 'info');
      const data = encodeSwapExactTokensForTokens(swapQuote.tokenIn, swapQuote.amountOutMin, swapQuote.path, primaryWallet.address, deadline);
      tx = await primaryWallet.sendTransaction({ to: PULSEX_ROUTER, data, gasLimit: 400000n, type: 0, gasPrice: swapGasPrice });
    }

    setStatus(st, 'Waiting for confirmation…', 'info');
    await tx.wait();
    const netFmt = (Number(swapQuote.stableBase) / 10 ** toTok.decimals).toLocaleString(undefined, { maximumFractionDigits: 2 });
    setStatus(st, `✓ Swapped → $${netFmt} ${toTok.label}. Now go to Shield to deposit.`, 'ok', tx.hash);
    // Reset UI
    document.getElementById('swap-dollar-input').value      = '';
    document.getElementById('swap-confirm').style.display   = 'none';
    document.getElementById('swap-quote-box').style.display = 'none';
    document.getElementById('swap-btn').style.display       = '';
    document.getElementById('swap-btn').disabled            = true;
    ...
    document.getElementById('swap-dollar-input').disabled   = false;
    document.getElementById('swap-token-grid').style.pointerEvents = '';
    document.querySelectorAll('.swap-denom-btn').forEach(b => b.classList.remove('active'));
    swapQuote = null;
    refreshHeader();
  } catch(e) {
